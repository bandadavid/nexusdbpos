...
namespace Ada369Csharp.Presentacion.Configuraciones
{
    public partial class Serializacionconfig : UserControl
    {
        ...
        {
            if (TXTCOMPRO.Text.Equals("") || TXTCANTIDADDECEROS.Text.Equals("") || txtnumerofin.Text.Equals("") || txtSerie.Text.Equals(""))
            {
                MessageBox.Show("Ingresa todos los datos para poder continuar", "Serializacion de Comprobantes", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            } 
            else
            {
                try
